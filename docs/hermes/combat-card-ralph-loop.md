...
- [x] Reward-overlay follow-through in the active combat stage: tighten victory/reward presentation so reward cards read as card objects first, and add/adjust smoke coverage around the reward-state presentation contract without depending on exact reward IDs.
- [x] Add one lightweight automated visual-verification slice for combat-stage readability or reward-overlay presence if it can be done without broad harness churn.
- [x] Polish the active combat-stage recent-event feed so it stays compact/readable beside the enlarged card presentation without breaking the current text contract.
...
- [x] Extend RewardDraft weighting so authored `weight_modifiers[]` can influence offers in a bounded deterministic way without disturbing existing fixture baselines.
...
- Autonomous slice complete: reward-overlay follow-through is now explicitly covered by a new headless smoke probe `tests/smoke/run_reward_overlay_probe.gd` plus `test_reward_overlay_presents_and_then_confirms_reward_state_cleanly`, asserting presented/applied reward-state presentation without hard-coding random offer ids. As part of the same pass, `CombatHudController` now points the reward seal and focus lane at the newer generated polish assets so the standard HUD overlay matches the active stage's upgraded art direction. Validation: targeted new smoke, full smoke module, full unittest discover, headless Godot startup, floor integration probe, and floor determinism probe all passed.
- Breaking-point check complete after the combat-stage board went fully green: fetched both `origin` and `upstream` and confirmed the branch was still current before continuing into the next slice.
- Autonomous slice complete: the active `CombatStageController` recent-event feed now uses a cleaner three-row layout with order badges, tone-aware row styling, and a highlighted latest row while preserving the underlying event text contract. A new headless probe `tests/smoke/run_combat_stage_event_feed_probe.gd` asserts panel size, row count, badge extraction, tone classification, and readable text truncation without brittle pixel testing. Manual screenshot review via vision confirmed the panel stays compact/readable beside the enlarged hand.
- Autonomous slice complete: runtime now consumes one more authored play-condition type, `discard_at_least`, via new proof card `probe_discard_ready_anchor`. `CardValidator`, `CombatSliceRunner`, HUD/stage reject copy, and `run_card_play_contract_probe.gd` all now cover the discard-threshold legality path. Validation: targeted play-contract smoke, full smoke module, full unittest discover, headless Godot startup, floor integration probe, and floor determinism probe all passed with determinism baselines unchanged.
- Autonomous slice complete: lifecycle-tag proof coverage now extends beyond discard/exhaust. New proof cards `probe_retain_anchor` and `probe_temp_anchor` exercise authored `zone_on_play` values `retain` and `temp`, and `run_card_play_contract_probe.gd` now asserts the retain card returns to hand while the temp card exhausts after play. Validation: targeted play-contract smoke, full smoke module, full unittest discover, headless Godot startup, floor integration probe, and floor determinism probe all passed with determinism baselines unchanged.
- Autonomous slice complete: `RewardDraft` now consumes a bounded runtime subset of authored `weight_modifiers[]` without broad schema invention. Current supported path is deterministic `multiply` modifiers with per-modifier/final clamping and optional explicit `reward_context.weight_modifier_conditions[]` activation keys. `run_reward_pool_probe.gd` and the smoke suite now prove unconditional clamp behavior plus condition-key activation, while determinism fixture baselines remained unchanged. Validation: targeted reward-pool smoke (RED->GREEN), full smoke module, determinism fixture compare, and full unittest discover all passed.
